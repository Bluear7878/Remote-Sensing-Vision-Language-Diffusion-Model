...
def run_pipeline(
    ...
    for row in tqdm(dataset):
        ...
        if task == "canny":
            assert _forward_kwargs.get("height", 1024) == 1024
            assert _forward_kwargs.get("width", 1024) == 1024
        ...
        gen_image.save(os.path.join(save_dir, f"{filename}.png"))
    ...
    print(
        f"Total inference time for {len(dataset)} samples: "
        f"{total_time:.2f} seconds"
    )
    print(
        f"Avg. inference time per sample: {avg_time:.2f} seconds"
    )
    return total_time, avg_time, iteration_times
...
def run_test(
    ...
    Double_FBCache: bool = True,
    residual_diff_threshold_multi: float = 0.0,
    residual_diff_threshold_single: float = 0.0,
    ...
    test_root = os.path.join("test_results")
    ref_root = os.path.join(test_root, "ref")
    bit4_root = os.path.join(test_root, f"4bit_{run_id}")

    os.makedirs(ref_root, exist_ok=True)
    os.makedirs(bit4_root, exist_ok=True)
    ...
    results_summary = {
        ...
        "lpips": None,
        "16bit_total_time": None,
        "16bit_avg_time": None,
        "4bit_total_time": None,
        "4bit_avg_time": None,
    }
    ...
    if not already_generate(save_dir_16bit, max_dataset_size):
        ...
        if len(lora_names) > 0:
            for i, (lora_name, lora_strength) in enumerate(zip(lora_names, lora_strengths)):
                ...
                pipeline.load_lora_weights(os.path.dirname(lora_path), weight_name=os.path.basename(lora_path), adapter_name=f"lora_{i}",
                )
            pipeline.set_adapters([f"lora_{i}" for i in range(len(lora_names))], lora_strengths)


        total_time_16, avg_time_16, _ = run_pipeline(
            dataset=dataset,
            task=task,
            pipeline=pipeline,
    ...
    save_dir_4bit = os.path.join(bit4_root, dtype_str, precision_str, model_name, folder_name)
    ...
    if not already_generate(save_dir_4bit, max_dataset_size):
        ...
        if len(lora_names) > 0:
            if len(lora_names) == 1: # directly load the lora
                ...
                composed_lora = compose_lora(
                    [
                        (LORA_PATH_MAP[lname], lstrength)
                        for lname, lstrength in zip(lora_names, lora_strengths)
                    ]
                )
        ...
        total_time_4, avg_time_4, _ = run_pipeline(
        ...
        results_summary["4bit_total_time"] = round(total_time_4, 2)
        results_summary["4bit_avg_time"] = round(avg_time_4, 2)
